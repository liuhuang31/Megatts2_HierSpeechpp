...
from ttv_v1.t2w2v_transformer import Megatts2PLM, Megatts2PLM1
...
def tts(text, a, hierspeech, prompt_path="", speaker_name="", item_count=1):
    ...
    net_g, text2w2v, speechsr, denoiser, mel_fn, plm = hierspeech
    ...
    with torch.no_grad():
        ...
        x_frame, g, x_lengths, x_mask = text2w2v.inf_extract_tc_latent(text, text_length, src_mel_ttv, src_mel_ttv_length, tone, language)
        codes = plm.infer(x_frame)
        # print(f"x_frame: {x_frame.shape} codes: {codes.shape}")
        w2v_x, pitch = text2w2v.inf_plm_gen(x_frame, g, codes.unsqueeze(1), x_lengths, x_mask)
    ...
    output_file = os.path.join(output_dir, "audio", str(item_count).zfill(3)+".wav")
...
def model_load(a):
    ...
    **hps_t2w2v.model).cuda()
    ...
    ## plm
    plm1 = Megatts2PLM1().cuda()
    plm1, _, _, _ = utils.load_checkpoint(a.ckpt_plm, plm1, None)
    plm1.eval()

    if a.output_sr == 48000:
        audiosr = SpeechSR48(h_sr48.data.n_mel_channels,
        ...
        audiosr = SpeechSR24(h_sr.data.n_mel_channels,
    ...
    denoiser.eval()
    return net_g, text2w2v, audiosr, denoiser, mel_fn, plm1
...
def main():
    ...
    parser.add_argument('--input_prompt', default='./filelists/prompt4.txt',help="a text file contains prompt wav dir")
    parser.add_argument('--input_txt', default='./filelists/reference_vctk.txt', help="a text file contains text")
    parser.add_argument('--output_dir', default='output/exp18_all_1300k_plm1_1550k_vctk_test')
    parser.add_argument('--ckpt', default='./logs/hierspeechpp_eng_kor/hierspeechpp_v1.1_ckpt.pth')
    parser.add_argument('--ckpt_text2w2v', '-ct', help='text2w2v checkpoint path', default='./logs/exp18/G_1300000.pth')
    parser.add_argument('--ckpt_plm', '-cp', help='plm checkpoint path', default='./logs/exp18/P1_1550000.pth')
...
CUDA_VISIBLE_DEVICES=0 python inference_plm.py
CUDA_VISIBLE_DEVICES=1 python inference_plm.py
CUDA_VISIBLE_DEVICES=2 python inference_plm.py
CUDA_VISIBLE_DEVICES=3 python inference_plm.py
'''