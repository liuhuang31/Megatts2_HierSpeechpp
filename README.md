...
CUDA_VISIBLE_DEVICES="2,3" python train_ms.py -c configs/config.json -m exp18

# exp_current train_s1, train GPT, not use GPT-SoVITS's AR modules model, we use github_megatts2's GPT model.
CUDA_VISIBLE_DEVICES="2,3" python train_ms_s1.py -c configs/config.json -m exp18
```
...
CUDA_VISIBLE_DEVICES=0 python3 inference.py  --ckpt "logs/hierspeechpp_libritts960/hierspeechpp_lt960_ckpt.pth" --ckpt_text2w2v "logs/ttv_libritts_v1/ttv_lt960_ckpt.pth"  --output_dir "tts_results_eng_kor_v2"  --noise_scale_vc "0.333"  --noise_scale_ttv "0.333"  --denoise_ratio "0"
```
