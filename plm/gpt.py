...
class GPT(nn.Module):
    ...
    def get_logits(
        ...
        if prompt is not None:
            ...
            if second_inputs is not None:
                ...
                emb = first_inputs
        print(f"first_inputs second_inputs emb: {first_inputs.shape} {second_inputs.shape} {emb.shape}")
        ...
        # when inf, 'attn_mask' is None
        print(f"get_logits: {get_attns} {attn_mask}")   #get_logits emb: torch.Size([1, 139, 1024]) False None
        gpt_out = self.gpt(
            inputs_embeds=emb,
            return_dict=True,
        ...
        # for we not use prompt
        # enc = gpt_out.last_hidden_state[:, offset:]
        enc = gpt_out.last_hidden_state
        print(f"enc1: {enc.shape}") #[1, 107, 1024]
        enc = self.final_norm(enc)
        print(f"enc2: {enc.shape}") #[1, 107, 1024]

        if return_latent:
            return enc[:, : first_inputs.shape[1]], enc[:, -second_inputs.shape[1] :]

        first_logits = enc[:, : first_inputs.shape[1]]
        print(f"first_logits1: {first_logits.shape}")   #[8, 85, 1024]
        first_logits = first_head(first_logits)
        print(f"first_logits2: {first_logits.shape}")   #[8, 85, 6153]
        first_logits = first_logits.permute(0, 2, 1)
        print(f"first_logits3: {first_logits.shape}")   #[8, 6153, 85]
        if second_inputs is not None:
    ...
    def forward(
        ...
        code_lengths = text_lengths #torch.ceil(wav_lengths / self.code_stride_len).long() + 3
        print(f"code_lengths1: {code_lengths}") # 91+3 = [94]
        ...
        print(f"cond_lens: {cond_lens}, cond_idxs: {cond_idxs}")    #cond_lens: None, cond_idxs: None
        ...
        print(f"code_lengths2: {code_lengths}") #[94]
        # 💖 Lovely assertions
        assert (
        ...
        text_targets = torch.cat((text_inputs, text_start), dim=1)
        text_inputs = torch.cat((text_start, text_inputs), dim=1)

        audio_codes, mel_targets = self.set_inputs_and_targets(
            audio_codes, self.start_audio_token, self.stop_audio_token
        )
        print(f"text_inputs: {text_inputs.shape}, text_targets: {text_targets.shape}")  #torch.Size([1, 24]), text_targets: torch.Size([1, 24]
        print(f"audio_codes: {audio_codes.shape}, mel_targets: {mel_targets.shape}")    #torch.Size([1, 96]), mel_targets: torch.Size([1, 96])
        # Set attn_mask
        attn_mask_cond = None
        attn_mask_text = None
        attn_mask_mel = None
        ...
        text_emb =  text_inputs#self.text_embedding(text_inputs) + self.text_pos_embedding(text_inputs)
        print(f"text_emb: {text_emb.shape}")    # [1, 24, 1024]
        # Compute mel embeddings + positional embeddings
        mel_emb = self.mel_embedding(audio_codes) + self.mel_pos_embedding(audio_codes)
        print(f"mel_emb: {mel_emb.shape}")  # [1, 96, 1024]
        ...
        print(f"text_logits: {text_logits.shape}, mel_logits: {mel_logits.shape}")  #[1, 24, 1024]), mel_logits: torch.Size([1, 96, 1024]
        if return_latent:
            print(f"mel_logits[:, :sub]: {mel_logits[:, :sub].shape}")  #mel_logits[:, :sub]: torch.Size([1, 88, 1024])
        ...
        print(f"text_logits, text_targets: {text_logits.shape} {text_targets.shape}")
        # Compute losses
        # loss_text = F.cross_entropy(
        #     text_logits, text_targets.long(), ignore_index=-1, label_smoothing=self.label_smoothing
        # )
        print(f"mel_logits, mel_targets: {mel_logits.shape} {mel_targets.shape}")
    ...
    def inference(self, text_inputs, **hf_generate_kwargs):
        self.compute_embeddings(text_inputs)
        return self.generate(text_inputs, **hf_generate_kwargs)
    ...
    def compute_embeddings(
        ...
        # self.stop_text_token:0, self.start_text_token:261 self.start_audio_token: 1024
        print(f"self.stop_text_token:{self.stop_text_token}, self.start_text_token:{self.start_text_token} self.start_audio_token: {self.start_audio_token}")
        print(f"text_inputs_before: {text_inputs.shape}")   #text_inputs_before: torch.Size([1, 22])
        # text_inputs = F.pad(text_inputs, (0, 1), value=self.stop_text_token)
        # text_inputs = F.pad(text_inputs, (1, 0), value=self.start_text_token)
        # print(f"text_inputs_after: {text_inputs.shape}")    #text_inputs_after: torch.Size([1, 24])
        # emb = self.text_embedding(text_inputs) + self.text_pos_embedding(text_inputs)

        print(f"text_inputs before cat: {text_inputs.shape}")  #emb1 before cat: torch.Size([1, 24, 1024])
        text_start = text_end = torch.randn(1, 1, 1024)
        emb = torch.cat((text_start, text_inputs, text_end), dim=1)
        
        # emb = torch.cat([cond_latents, emb], dim=1)
        print(f"emb1 after cat: {emb.shape}")   # emb1 after cat: torch.Size([1, 56, 1024])
        ...
        print(f"gpt_inputs: {gpt_inputs.shape}")    # gpt_inputs: torch.Size([1, 57])
    ...
    def generate(
        ...
        **hf_generate_kwargs,
        ...
        gpt_inputs = self.compute_embeddings(text_inputs)
...
if __name__ == '__main__':
    gpt = GPT(model_dim=1024)

    text_emb = torch.randn(1, 2, 1024)
    text_lengths = torch.LongTensor([text_emb.shape[1]])
    audio_codes = torch.randint(low=2, high=100, size=(10,), dtype=torch.int32)
    audio_codes = audio_codes.unsqueeze(0)
    a = gpt(text_emb, text_lengths, audio_codes)
    
    ## inference
    gpt.init_gpt_for_inference()
    bb = gpt.generate(text_emb)
    print(f"bb: {bb.shape} {bb}")
