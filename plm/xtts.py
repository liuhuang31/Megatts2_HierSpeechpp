...
class Xtts(BaseTTS):
    ...
    def get_conditioning_latents(
        ...
        print(f"full_audio: {full_audio.shape}")
    ...
    def inference(
        ...
        **hf_generate_kwargs,
        ...
        '''
        > Input text:  stay cool he added with a smile.
        > Input text preprocessed:  stay cool he added with a smile.
        > Input tokens:  tensor([[ 259,   63,   88,    2,   16, 2751,   25,    2,   62,    2,   68,   17,
                49,    2,  103,    2,   14,    2, 3139,   22,   64,    9]],
            device='cuda:0', dtype=torch.int32)
        > Decoded text:  [en]stay cool he added with a smile.
        '''
        ...
        with torch.no_grad():
            ...
            '''
            gpt_codes: torch.Size([1, 87]) tensor([[1012, 1011, 1011, 1005,  170,  743,   28,  225,  256,   40,   40,  325,
            432,  856,  367,  784,  640,  627,   22,  466,  699,   30,  559,  419,
             74,  545,  244,  524,  784,  567,  643,  567,  492,  988, 1002, 1006,
           1007,  993,  985,  720,  701,  270,   25,   59,  323,  655,  630,  527,
            613,  144,  405,   61,  369,  841,  281,  493,  110,  115,  481,  406,
            370,  692,  290,  248,  243,  551,  974,  972,  980,  981, 1002,    0,
            414,  193,  310,  623,  838,  811,  253, 1004, 1008, 1011, 1011, 1011,
           1011, 1011, 1025]], device='cuda:0')
            '''
            print(f"gpt_codes: {gpt_codes.shape} {gpt_codes}")
            expected_output_len = torch.tensor(
                [gpt_codes.shape[-1] * self.gpt.code_stride_len], device=text_tokens.device
            )

            text_len = torch.tensor([text_tokens.shape[-1]], device=self.device)
            print(f"expected_output_len: {expected_output_len}, text_len: {text_len}")  # [89088] text_len: [22]
            ...
            print(f"gpt_latents: {gpt_latents.shape}")  # gpt_latents: torch.Size([1, 87, 1024]
            silence_token = 83
            ctokens = 0
